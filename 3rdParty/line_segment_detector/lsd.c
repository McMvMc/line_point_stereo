 ...
 */
static image_double ll_angle_masked( image_double in, struct coorlist * mask,
                              double threshold,
                              struct coorlist ** list_p, void ** mem_p,
                              image_double * modgrad, unsigned int n_bins )
{
...
/*----------------------------------------------------------------------------*/
/** LSD full interface with additional mask.
 */
double * MaskedLineSegmentDetection( int * n_out,
                               double * img, int X, int Y,
                               ...
                               int ** reg_img, int * reg_x, int * reg_y )
{
  ...
  /* check parameters */
  if( img == NULL || X <= 0 || Y <= 0 ) error("invalid image input.");
  if( mask == NULL ) error("invalid mask input.");
  if( scale <= 0.0 ) error("'scale' value must be positive.");
  if( sigma_scale <= 0.0 ) error("'sigma_scale' value must be positive.");
  if( quant < 0.0 ) error("'quant' value must be positive.");
  if( ang_th <= 0.0 || ang_th >= 180.0 )
  ...
  if( scale != 1.0 )
    {
      scaled_image = gaussian_sampler( image, scale, sigma_scale );
      angles = ll_angle_masked( scaled_image, mask, rho, &list_p, &mem_p,
                         &modgrad, (unsigned int) n_bins );
      free_image_double(scaled_image);
    }
  else
    angles = ll_angle_masked( image, mask, rho, &list_p, &mem_p, &modgrad,
                       (unsigned int) n_bins );
  xsize = angles->xsize;
  ysize = angles->ysize;
